...
const { verifyProof } = require('../dist/verifyProof');
const {
  getFormDataError,
  verifySignature,
  toSignatureString,
...
const mockChain = {
  ...
  // No demo signer is a contract wallet, so the EIP-1271 path is never taken here.
  async isContract() { return false; },
  async isValidErc1271() { return false; },
};
...
// The SAME verifier the real server runs (src/verifyProof.ts), against the mock chain — a
// second copy here is what let the two drift apart in the first place.
app.post('/verify', async (req, res) => {
  const out = await verifyProof({ cfg, chain: mockChain, eligibility }, req.body);
